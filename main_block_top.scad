...
mount_spacing = 150; 	// use 30 if using the newer, 30mm hole x-carriage
jhead_mount = false; 	// set to false if using prusanozzle or stacking
filament = 3;			// set to 3mm or 1.75mm or custom
drive = "front";		    // 'front' = front filament drive
...
difference(){
	union(){
		...
		translate([-2.99,-42,0])cube([3,61,52]);  	// base
